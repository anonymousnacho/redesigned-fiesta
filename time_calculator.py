def add_time(start, duration, day_of_week = False):

  days_index = {"monday": 0, "tuesday": 1, "wednesday": 2, "thursday": 3, "friday": 4, "saturday": 5, "sunday": 6}

  days_array = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]

  duration_tuple = duration.partition(":") #separates values with colon bc time
  print(duration_tuple)
  duration_hours = int(duration_tuple[0])
  duration_min = int(duration_tuple[2])

  start_tuple = start.partition(":")
  start_minutes_tuple = start_tuple[2].partition(" ") #adds space bw time and AM/PM
  start_hours = int(start_tuple[0])
  start_minutes = int(start_minutes_tuple[0])
  am_or_pm = start_minutes_tuple[2]
  am_pm_flip = {"AM" : "PM", "PM" : "AM"}

  amount_days = int(duration_hours / 24)

  end_minutes = start_minutes + duration_min
  if end_minutes > 60:
    start_hours += 1 #adds to hours if minutes exceed 60 bc time
    end_minutes = end_minutes % 60 
  amount_of_flips = int((start_hours + duration_hours) / 12)
  end_hours = (start_hours + duration_hours) % 12
    
  end_minutes = end_minutes if end_minutes > 9 else "0" + str(end_minutes)
  end_hours = end_hours = 12 if end_hours == 0 else end_hours

  if(am_or_pm == "PM" and start_hours + (duration_hours % 12) >= 12):
    amount_days += 1 #rolls over a new day if needed

  am_or_pm = am_pm_flip[am_or_pm] if amount_of_flips % 2 == 1 else am_or_pm
    
  returnTime = str(end_hours) + ":" + str(end_minutes) + " " + am_or_pm

  if day_of_week: #if a dotw is entered this handles that
    day_of_week = day_of_week.lower()
    index = int((days_index[day_of_week]) + amount_days) % 7
    new_day = days_array[index]
    returnTime += ", " + new_day

  if amount_days == 1:
    return returnTime + " " + "(next day)"
  elif amount_days > 1: 
    return returnTime + " (" + str(amount_days) + " days later)"
    
    
  return returnTime
